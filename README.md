...
4. **Scale** — `--scale_method stature` (default): with MeTRAbs, the height of the head-band markers above the floor (lowest sole markers) is measured over the walk (95th centile) and set to **0.9255 × `--height`**, a ratio fixed once on the BioCV development set. On four held-out datasets the median scale error is **1.0 %**, against 1.9 % for `--scale_method segments` (thigh + shank = 0.491 × `--height`, Drillis & Contini), which lets each person's leg proportions into the scale. RTMPose, whose skeleton has no head-band markers, uses the segments (thigh + shank + trunk = 0.779 × `--height`). `--scale_method head` (one frame) is kept for reference only (about 11 % on BioCV).

...
| 2026-09-21 | **Échelle MeTRAbs : hauteur tête → sol au lieu des jambes × 0,491** (`--scale_method stature`, défaut). Bandeau de tête MeTRAbs (4 marqueurs virtuels) au-dessus du marqueur de semelle le plus bas, 95ᵉ centile sur la marche, = 0,9255 × stature ; rapport fixé sur BioCV seul, appliqué sans retouche ailleurs. 77 calibrations recalculées : hors BioCV |erreur| médiane 1,89 → 1,04 % (p = 9·10⁻⁵), IMOVE +2,3 → +1,6 %, LBMC +4,0 → +2,0 %, COMFI +1,9 → +0,1 %, OpenCap +1,0 → +0,6 % ; rotations inchangées. RTMPose : sommet du crâne Halpe26 sans gain, garde jambes + tronc. Chaussures et posture de marche écartées comme cause du résidu IMOVE/LBMC (vidéos ; APEX mocap IMOVE à 0,99 × stature en marche). Détail : `docs/article/03_RESULTATS.md` §20. |
